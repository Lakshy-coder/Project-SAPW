...
describe('AuditService', () => {
  ...
  it('throws when sealing a job with no events', async () => {
    await expect(svc.seal('unknown-job')).rejects.toThrow('PROVENANCE_ERROR');
  });

  it('seals a receipt and verifies it', async () => {
    svc.emit('job-2', 'START', { input: 'x' }, { out: 'y' });
    svc.emit('job-2', 'END',   { input: 'z' }, { out: 'w' });
    const receipt = await svc.seal('job-2');
  ...
  it('detects tampered receipt', async () => {
    ...
    const receipt = await svc.seal('job-3');
    // Tamper a block hash
